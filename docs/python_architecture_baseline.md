...
This document records the current root-level Python runtime after the package
restructure.
...
The legacy Java prototype and wrapper `python/` directory are not part of this
runtime.
...
| `workflow/` | End-to-end run orchestration for browsing, generation, execution, repair, and reporting. |
...
7. The journey guide is written before test generation.
8. The generation prompt is sent with the replay plan and structured contract.
9. The generated test is written under `generated-tests/`.
10. The generated test is executed through `core/executor.py`.
11. The execution report is written under `test-results/`.
...
Current coverage is conservative:
...
1. Backend evidence is limited to frontend-visible HTTP traffic.
2. External report formats such as JUnit XML are not generated.
3. The browse and generation phases share the same agent runtime.
