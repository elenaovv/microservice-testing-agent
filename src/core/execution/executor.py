...
from core.contracts.models import ExecutionArtifact, ExecutionResult
from core.execution.run_artifacts import RUNTIME_RESULTS_DIR
from core.execution.runtime_env import build_subprocess_env
...
def _build_pytest_args(runtime_results_dir: Path) -> list[str]:
    cache_dir = runtime_results_dir / ".pytest_cache" / str(os.getpid())
    return [
        *PYTEST_ARGS_BASE,
        "-o",
        f"cache_dir={cache_dir}",
        "--output",
        str(runtime_results_dir),
    ]
...
def run_generated_test(
    ...
    result = subprocess.run(
        ["uv", "run", "pytest", *_build_pytest_args(resolved_runtime_results_dir), str(test_path)],
        capture_output=True,
        env=env,
        text=True,
        check=False,
